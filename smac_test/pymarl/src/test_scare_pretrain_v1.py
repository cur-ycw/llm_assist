"""
SCARE Encoder Pre-training Workflow Test (SMAC v1 — 3m map)
分步测试 LLM 生成语义 -> 奖励函数 -> 训练 encoder 的完整流程 (3v3 marines)

Usage:
    conda activate pymarl
    cd D:/ClaudeCodeRepo/smac_test/pymarl
    python src/test_scare_pretrain_v1.py
"""
...
# ============================================================
# Prompt templates for 3m (auto-generated from map metadata)
# ============================================================
from scare_pretrain import build_prompts_for_map

_prompts_3m = build_prompts_for_map("3m")
PROMPT_ENV = _prompts_3m['env']
PROMPT_CODE = _prompts_3m['code']
...
def test_step1_llm_generate_behavior():
    print_step(1, "LLM 生成 cooperation preference (3m)")
...
def test_step2_llm_write_reward(behavior):
    print_step(2, "LLM 生成 reward function 代码 (3m)")
...
# ============================================================
# Step 4: Test QMIX training with sc2_scare wrapper (short run)
# ============================================================
def test_step4_qmix_training():
    print_step(4, "QMIX 短训练测试 with sc2_scare wrapper (t_max=5000)")
    ...
    cmd = (
        ...
        f'--config=qmix --env-config=sc2_scare '
        f'with t_max=5000 '
        f'test_interval=2000 '
        f'test_nepisode=2 '
        f'save_model=True '
        f'save_model_interval=2000 '
        f'name=scare_v1_encoder_test '
        f'batch_size=4 '
        f'buffer_size=100 '
        f'env_args.additional_reward_id=0 '
    )
...
def test_step5_load_into_scare(model_path):
    ...
    args = SimpleNamespace(
        ...
        n_encoders=4,
        enc_feature_dim=rnn_state_dict['fc1.weight'].shape[0],
...
if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("  SCARE Encoder Pre-training Workflow Test (SMAC v1 — 3m)")
    print("=" * 60)
    ...
    if not results['step1_behavior']:
        behavior = "Human players may prefer to focus fire on enemy A together."
    ...
    if not results['step2_code']:
        ...
        def additional_reward(self) -> float:
            ...
            d3 = self.agent_enemy_distance("3", "A")
            reward -= (d1 + d2 + d3) * 0.1
    ...
    # Step 4: QMIX short training with sc2_scare
